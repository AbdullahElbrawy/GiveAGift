...
export default function CustomTextImgSection({
	...
	return (
		<Container
			component="section"
			className={`!flex ${reverse ? "flex-row-reverse" : ""} justify-between flex-wrap *:rtl:text-right`}
		>
			<div className={`${className ? "w-full" : "md:w-6/12  w-full h-full"} flex items-center justify-center`}>
				<img
					src={imgUrl}
					alt={title}
					className={`object-cover rounded ${secondary ? "h-52 md:h-auto" : "h-[300px]"} w-full`}
				/>

				{textOn && (
					<div className="absolute   items-center justify-center mt-5">
						<div className="p-4 bg-primary-500/75 flex flex-col   rounded-2xl shadow-2xl flex justify-center">
							<Typography
								variant="h2"
								component="h1"
			...
			<div className="flex flex-col justify-center md:w-5/12 w-full !text-center lg:!text-start mt-5 lg:mt-0">
				<Typography
					variant="h3"
					component="h2"
					className={`${!secondary ? "text-primary-700" : "text-rose-700"} !mb-6 sm-mt-5`}
				>
